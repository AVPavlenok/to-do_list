...
from src.crud import create_item, get_item, patch_item, delete_item


def create_app(engine_db, session_db):
    @asynccontextmanager
    ...
    @application.post('/register', status_code=status.HTTP_201_CREATED, summary="Create a new user")
    async def register(user: UserCreate):
        user.password = await hash_password(user.password)
        try:
            await create_item(model=User, session=session_db, item=user.model_dump())
        except IntegrityError:
            raise HTTPException(detail='Such user already exists!', status_code=status.HTTP_409_CONFLICT)
        except Exception as ex:
            raise HTTPException(detail=f'Try again later! {ex}', status_code=status.HTTP_404_NOT_FOUND)
        return "You successfully registered!"

    @application.post('/login', status_code=status.HTTP_200_OK, summary='Login for using application')
    async def login(user: UserBase, response: Response):
        user_db = await get_item(model=User, session=session_db, login=user.login)
        if not user_db:
    ...
    @application.post('/logout', status_code=status.HTTP_200_OK, summary='Logout from our service')
    async def logout(response: Response):
        response.delete_cookie(key="access_token", httponly=True)
        return "You seccessfully logout!"

    @application.post('/tasks', status_code=status.HTTP_201_CREATED, summary='Create a new task')
    async def create_task(task: TaskIn, user_id: int = Depends(get_user_id)):
        try:
            await create_item(model=Task, session=session_db, item={'task': task.task, 'user_id': user_id})
        except Exception:
    ...
    @application.get(
        ...
        summary='List of all my tasks'
    )
    async def get_all_my_tasks(user_id: int = Depends(get_user_id)):
        tasks = await get_item(model=Task, session=session_db, user_id=user_id)
        return tasks
    ...
    @application.get(
        ...
        response_model=List[TaskOut],
        status_code=status.HTTP_200_OK,
        summary='List of actual tasks'
    )
    async def get_my_actual_tasks(user_id: int = Depends(get_user_id)):
        tasks = await get_item(model=Task, session=session_db, user_id=user_id, is_complete=False)
        return tasks
    ...
    async def get_task_by_id(task_id: int, user_id: int = Depends(get_user_id)):
        task = await get_item(model=Task, session=session_db, user_id=user_id, id=task_id)
        if not task:
    ...
    async def delete_task(task_id: int, user_id: int = Depends(get_user_id)):
        try:
            await delete_item(model=Task, session=session_db, user_id=user_id, id=task_id)
        except Exception:
            raise HTTPException(detail='No such task', status_code=status.HTTP_404_NOT_FOUND)
